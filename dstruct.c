...
void generatePseudoCode(CARD *cardList,FILE *outFile_p)
{
 while(cardList)
 {
  ...
  while(tempColbr) {
    switch(tempColbr->relation) {
         case NONE:
            fprintf(outFile_p,"// This Class has no relation specified\n");
            break;
         case USING_R:
            fprintf(outFile_p,"// This Class USES class %s\n", tempColbr->className);
            break;
         case HAS_A_R:
            fprintf(outFile_p,"// This Class HAS A class %s\n", tempColbr->className);
            break;
         case KIND_OF_R:
            fprintf(outFile_p,"// This Class is a KIND OF class %s\n", tempColbr->className);
            break;
         default:
            break;
       }
